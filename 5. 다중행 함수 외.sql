...
SELECT 
    ...
    , COUNT(direct_manager_emp_no) "dmen" -- null은 세지 않음
    , COUNT(*) 
...
SELECT 
    ...
    --, ROUND(pay_amt, 2)
FROM tb_sal_his
GROUP BY emp_no --GROUP BY에서 그룹함수를 쓰지 않고 집계함수에 속하지 않은 일반함수는 SELEC에 넣을 수 없음
ORDER BY emp_no
;
...
-- 사원별로 2019년에 급여를 제일 많이받았을 때, 제일 적게 받았을 때, 평균적으로 얼마받았는지 조회
-- GROUP BY 절에 있는 WHERE절은 그룹화 하기 전 필터링하는 것
SELECT 
    emp_no "사번"
    , TO_CHAR(MAX(pay_amt), 'L999,999,999') "최고 수령액"
    , TO_CHAR(MIN(pay_amt), 'L999,999,999') "최저 수령액"
